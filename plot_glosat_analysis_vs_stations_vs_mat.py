...
#------------------------------------------------------------------------------
# PROGRAM: plot_glosat_analysis_vs_stations_mat.py
#------------------------------------------------------------------------------
# Version 0.5
# 7 March, 2023
# Michael Taylor
...
filename_count_analysis = 'OUT/df_count_analysis.pkl'
filename_count_mat = 'OUT/df_count_mat.pkl'
...
filename_gmst_analysis = 'OUT/df_gmst_analysis.pkl' 
filename_gmst_mat = 'OUT/df_gmst_mat.pkl' 
filename_gmst_lat = 'OUT/df_gmst_lat.pkl' 
filename_gmst_crutem_gridded = 'OUT/df_gmst_crutem_gridded.pkl' 
ncfile_analysis = 'DATA/glosat-analysis/GloSATref.1.0.0.0-alpha.4.analysis.analysis.anomalies.ensemble_median.nc' # infilled ens median
ncfile_mat = 'DATA/glosat-mat/GloSATMAT_2.4.0.0_anomaly_b1961_1990.nc'
...
use_cmocean = False         # [True,False] False --> 'bwr' 
use_cyan = True             # [True,False] False --> 'grey'
use_lat = False             # [True,False] False --> use alternative grid LAT instead of WGS84 weighted LAT
...
#datastr1 = r'$\bf{Data}$' + ' (analysis □): GloSAT.analysis.alpha.4 (UKMO-HC)'        
#datastr2 = r'$\bf{Data}$' + ' (mat ▫): GloSATMAT.2.4.0.0 ensemble mean (NOC)'  
#datastr3 = r'$\bf{Data}$' + ' (stations ○): GloSAT.p04c.EBC.LEKnormals (CRU/UEA, UYork)'  
datastr4 = r'$\bf{Data}$' + ' (NAO): 1781-1820 (Juerg Luterbacher), 1821-2022 (Phil Jones)'
datastr5 = r'$\bf{Data}$' + ' (SOI): 1866-2022 (Phil Jones)'
...
#------------------------------------------------------------------------------    
# LOAD: GMST(analysis) monthly timeseries calculated from area-averaged analysis + WGS84 area-weights
#------------------------------------------------------------------------------

print('loading GMST (analysis) timeseries ...')

# LOAD: pkl dataframe of monthly GMST(analysis)
     
df_gmst_analysis = pd.read_pickle( filename_gmst_analysis, compression='bz2' )      

#------------------------------------------------------------------------------    
# LOAD: GMST(MAT) monthly timeseries calculated from area-averaged MAT + WGS84 area-weights
#------------------------------------------------------------------------------

print('loading GMST (MAT) timeseries ...')

# LOAD: pkl dataframe of monthly GMST(MAT)
     
df_gmst_mat = pd.read_pickle( filename_gmst_mat, compression='bz2' )      
...
df_count_analysis = pd.read_pickle( filename_count_analysis, compression='bz2' )      
df_count_mat = pd.read_pickle( filename_count_mat, compression='bz2' )      
...
#----------------------------------------------------------------------------
# LOAD: GloSAT.analysis.alpha.4 Xarray
#----------------------------------------------------------------------------
    
print('loading analysis ...')
        
ds_analysis = xr.open_dataset( ncfile_analysis, decode_cf=True )
par_analysis = ds_analysis.tas_median

#--------------------------------------------------------------------------
# LOAD: GloSATMAT.2.4.0.0 Xarray
#--------------------------------------------------------------------------
...
for year in np.arange(year_start,year_end):
    ...
    for j in range(12):
        ...
        # EXTRACT: analysis 5x5 gridded anomaly background for given year and month
 
        v_analysis = par_analysis[k,:,:]
                    
        # EXTRACT: GloSATMAT for given year and month
        ...
        mask = np.isfinite(v_mat).values        
        x_mat, y_mat = np.meshgrid( v_mat.where(mask).longitude.values, v_mat.where(mask).latitude.values )
        X_mat = x_mat.ravel() 
        Y_mat = y_mat.ravel() 
        Z_mat = v_mat.where(mask).values.ravel()
        '''
        ...
        Y_mat = []
        Z_mat = []
        for mat_lat_idx in range( v_mat.shape[0] ):
            for mat_lon_idx in range( v_mat.shape[1] ):

                X_mat.append( v_mat[ mat_lat_idx, mat_lon_idx ].longitude.values+0 )
                Y_mat.append( v_mat[ mat_lat_idx, mat_lon_idx ].latitude.values+0 )
                Z_mat.append( v_mat[ mat_lat_idx, mat_lon_idx ].values+0 )                

        # FILTER: missing 

        da = pd.DataFrame({'X':X_mat, 'Y':Y_mat, 'Z':Z_mat}).dropna()
        ...
        count_analysis = df_count_analysis[df_count_analysis.index.year==year].values.ravel()[j]
        count_mat = df_count_mat[df_count_mat.index.year==year].values.ravel()[j]
        ...
        gmst_analysis = df_gmst_analysis[df_gmst_analysis.index.year==year]['gmst'].values.ravel()[j]
        gmst_mat = df_gmst_mat[df_gmst_mat.index.year==year]['gmst'].values.ravel()[j]
        ...
        titlestr = str(year) + '-' + str(j+1).zfill(2) + ' NAO=' + str( np.round( nao, 3 ) ) + ' SOI=' + str( np.round( soi, 3 ) ) + ' GMST (analysis)=' + str( np.round( gmst_analysis, 3 ) ) + ' GMST (MAT)=' + str( np.round( gmst_mat, 3 ) ) + ' GMST (LAT)=' + str( np.round( gmst, 3 ) ) 
        figstr = 'tas_median_' + str(year) + '-' + str(j+1).zfill(2) +'.png'        

        datastr1 = r'$\bf{Data}$' + r' (analysis □): GloSATref.1.0.0.0-alpha.4 infilled ens median (UKMO-HC)' + ': N=' + str(count_analysis) + '/' + str(n_lat*n_lon)
        datastr2 = r'$\bf{Data}$' + r' (MAT ▫): GloSATMAT.2.4.0.0 ens median (NOC)' + ': N=' + str(count_mat) + '/' + str(n_lat*n_lon)
        datastr3 = r'$\bf{Data}$' + r' (LAT ○): GloSAT.p04c.EBC.LEKnormals (CRU/UEA, UYork)' + ': N=' + str( count ) + '/' + str(n_lat*n_lon) + ' N(stations)=' + str( count_stations ) 
        ...
        # PLOT: 5x5 gridded analysis
    
        g_analysis = v_analysis.plot(ax=ax, alpha=1, vmin=vmin, vmax=vmax, cmap=cmap, transform=ccrs.PlateCarree(), cbar_kwargs={'orientation':'horizontal','extend':'both','shrink':0.5, 'pad':0.05}, zorder=10)                 
        cb = g_analysis.colorbar; cb.ax.tick_params(labelsize=fontsize); cb.set_label(label=r'Anomaly (from 1961-1990), $^{\circ}$C', size=fontsize); cb.ax.set_title(None, fontsize=fontsize)
        ...
        if np.isnan(Z).sum() != len(Z): h1 = plt.scatter(x=X, y=Y, c=Z, s=2, marker='o', edgecolor='k', lw=0.2, alpha=1, vmin=vmin, vmax=vmax, cmap=cmap, transform=ccrs.PlateCarree(), zorder=10000)         
    
        # PLOT: MAT gridcell center value
       
        if np.isnan(Z_mat).sum() != len(Z_mat): h2 = plt.scatter(x=X_mat, y=Y_mat, c=Z_mat, s=2, marker='s', edgecolor='k', lw=0.2, alpha=1, vmin=vmin, vmax=vmax, cmap=cmap, transform=ccrs.PlateCarree(), zorder=20000)         
            
        if dpi == 144: xstart = 330; ystart=10; ystep = 20
        elif dpi == 300: xstart = 680; ystart=10; ystep = 40
        elif dpi == 600: xstart = 1400; ystart=10; ystep = 80
        
        plt.annotate(datastr1, xy=(xstart,ystart+ystep*6), xycoords='figure pixels', color=default_color, fontsize=8)  
        plt.annotate(datastr2, xy=(xstart,ystart+ystep*5), xycoords='figure pixels', color=default_color, fontsize=8) 
        plt.annotate(datastr3, xy=(xstart,ystart+ystep*4), xycoords='figure pixels', color=default_color, fontsize=8) 
        plt.annotate(datastr4, xy=(xstart,ystart+ystep*3), xycoords='figure pixels', color=default_color, fontsize=8) 
        plt.annotate(datastr5, xy=(xstart,ystart+ystep*2), xycoords='figure pixels', color=default_color, fontsize=8) 
